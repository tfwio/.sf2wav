...
namespace on.soundfont_2_41
{
	...
	public partial class SoundFont2
	{
		...
		public void init_logging(string fname)
		{
			outp = string.Empty;
				...
				switch (foo)
				{
					case "INFO":
						...
						foreach (KeyValuePair<long, INFOsub> mox in nfo.nfosub)
						{
							...
							switch (mox.Value.Type.ToString4())
							{
								case "ifil":
									po = fstream.Position;
								...
								case "INAM":
									...
									break;
								default:
									// none of this information is actually used.
									po = fstream.Position;
									nx = new ZSTR();
									fstream.Seek(mox.Key+8, SeekOrigin.Begin);
		...
		public void init(string fname)
		{
			...
			foreach (KeyValuePair<long,SoundFont2.RIFFsub> rsx in riff.rss)
			{
				...
				{
          //case "sdta":
          //  System.Windows.Forms.MessageBox.Show($"sdta - {rsx.Value.Tag}");
          //  break;
					case "INFO":
						len = rsx.Value.Length;
						nfo = new INFO(len,bread,fstream);
