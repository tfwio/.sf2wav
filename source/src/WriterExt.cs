...
namespace System.IO
{
  ...
  /// <summary>
  /// this extension class singleton class should generally be used by  
  /// </summary>
	static class WriterExt
	{
    // static int Flip32(this BinaryReader r, int len) { return BitConverter.ToInt32(r.ReadBytes(len).flip(), 0); }
		static public byte[] flip(this byte[] input) { Array.Reverse(input); return input; }
    static public void WriteE(this BinaryWriter w, int input) { var bs = BitConverter.GetBytes(input); w.Write(BitConverter.IsLittleEndian ? bs : bs.flip(), 0, 4); }
    static public void WriteE(this BinaryWriter w, uint input) { var bs = BitConverter.GetBytes(input); w.Write(BitConverter.IsLittleEndian ? bs : bs.flip(), 0, 4); }
    static public void WriteE(this BinaryWriter w, ushort input) { var bs = BitConverter.GetBytes(input); w.Write(BitConverter.IsLittleEndian ? bs : bs.flip(), 0, 2); }
    static public void WriteE(this BinaryWriter w, short input) { var bs = BitConverter.GetBytes(input); w.Write(BitConverter.IsLittleEndian ? bs : bs.flip(), 0, 2); }
		static public void WriteE(this BinaryWriter w, byte input) { w.Write(input); } static public void WriteE(this BinaryWriter w, sbyte input) { w.Write(input); }
